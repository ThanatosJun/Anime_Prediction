...
This note records the first completed `c3_source_exact_pipeline.py` run.

## Run

- Run directory: `.exp/baseline/results/v2_source_exact_c3_pop_urgent_k64`
...
This run should not be promoted as the final C3 external baseline. It is useful as a diagnostic result showing that a more source-faithful staged SKAPP pipeline is currently unstable under the anime-domain mapping. The failure mode is not merely low accuracy: predictions collapse toward clipping boundaries for a large fraction of the test set.
...
- Future work should rerun the source-faithful path with `top_k=500`, both targets, calibration checks, and possibly a target-space/loss redesign before treating it as a final SKAPP reproduction.
