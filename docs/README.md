...
- **Reference baseline / 文獻對照實驗**
  - `../reports/baselines/reference_baseline_runs.md`：第一入口，列出已跑 route、run 來源與目前結論。
  - `../reports/baselines/reference_baseline_results.csv`：可攜帶的數字結果表，取代本機 `.exp/` raw outputs。
  - `../reports/baselines/reference_baseline_paper_alignment_audit.md`：判斷 C1/C2/C3 是否能主張對齊論文，避免把 proxy 寫成 reproduction。
  - `../reports/baselines/reference_baseline_status.md`：詳細工作紀錄與歷史脈絡，只有需要追溯決策時再看。
- **論文方法章與處理細節**
