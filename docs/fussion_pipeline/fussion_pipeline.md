...
### 1. Fusion Metadata 欄位更新
來源從 `anilist_anime_data_processed_v1.csv` 改為分 split 的 `anilist_anime_data_processed_v1_{split}.csv`，欄位從 28 欄擴充為 **33 欄**。

**新增欄位：**
| 欄位 | 說明 |
|------|------|
| `type` | 動畫類型（TV / Movie / OVA 等） |
| `release_date` | 完整日期字串 |
| `release_quarter_key` | 季度 key（如 2023Q1） |
| `voice_actor_names` | `|` 分隔的聲優名單 |
| `is_source_missing` | source 欄位是否缺值旗標 |

**排除欄位（post-release leakage）：**
`averageScore`, `favourites`, `trending`, `status`

**排除欄位（split 輔助）：**
`split_pre_release`, `split_pre_release_effective`, `is_model_split`, `is_title_native_imputed`, `is_startDate_day_missing`

### 2. 缺值處理
| 欄位 | 處理方式 |
|------|---------|
| `startDate_month` | 以 `season` 回推（SPRING→4, SUMMER→7, FALL→10, WINTER→1） |
| `release_date` | 同上，以 season + year 補值 |

### 3. voice_actor_names 暫時棄用
目前 `voice_actor_names` 缺值率約 **40%**，且聲優名字不適合直接做文字 embedding（all-MiniLM 不具備專有名詞語意）。

**本版本**：暫時從 fusion_meta 中保留欄位但不納入模型輸入。

**TODO（v4）**：改為 top-K 聲優 multi-hot encoding，邏輯同 studios multi-hot：
- 統計 training set 出現頻率前 K 名聲優（建議先測 top-50）
- 每部動畫編為 K-dim 二元向量
- 缺值填全 0（語意明確）
- 加入 MetaEncoder，input_dim 對應增加 K

### 4. 模型架構更新
```
輸入維度：text(384) + image(1024) + meta_rag(~158) = ~1566-dim

hidden_dims: [512, 256, 128]   ← 三層漸進壓縮
dropout:     0.4
```
...
### 5. 評估指標更新
popularity 新增兩個補充指標，解決原始 MAE 因數值範圍（25–231K）過大難以解讀的問題：

| 指標 | 說明 |
|------|------|
| `MAPE` | 平均絕對百分比誤差，scale-free |
| `log_MAE` | log 空間 MAE，對應模型實際優化的空間 |
| `bucket_accuracy` | 以訓練集四分位數劃分 4 個桶，分類準確率 |

### 6. Image Embedding 路徑
```
