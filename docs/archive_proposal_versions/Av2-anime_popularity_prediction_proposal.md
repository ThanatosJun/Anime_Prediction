...
| **表格元資料（Tabular）** | 製作公司等級、是否續作、原作類型、季度競爭強度 | AniList / MAL | 結構化特徵（季度競爭強度定義為：同季上檔 TV 動畫之總數量） |
| **IP 歷史特徵（Tabular）** | 前作 popularity（最終靜態值）、前作 score | AniList API | 無前作則填補 0，並增加 `is_original` 布林變數（0/1）作為標記 |

#### IP 知名度特徵之處理原則

- IP 特徵與目標變數相關性高，故不可省略
- 需透過消融實驗檢驗其與內容模態之相對貢獻
- 針對原創動畫（無前作），統一給予 `is_original=1` 標記並將歷史數據補 0，以確保回歸模型能獨立學習原創屬性之權重。
...
- **終點**：2025 年末（2025 年秋季番，確保所有樣本皆已擁有完整的 Day7 觀看數據，避免時間洩漏問題）
- **預計規模**：約 27 季、1,300+ 部 TV 動畫（以可取得 Day7 樣本為準）
...
基準模型之特徵融合（Fusion）方式統一採用 **Early Fusion（特徵拼接）**，將各模態經過 Encoder 提取出向量後直接串接（Concatenate），再輸入決策層 MLP。

兩個模型（Popularity、Score）各自進行以下實驗：

| 實驗組 | 輸入特徵 | 目的 |
|---|---|---|
| **E0: Heuristic Baseline** | 僅前作人氣/評分 或 製作公司歷史均值 | 建立最簡單的經驗法則對照組（Dummy Baseline） |
...
**核心假設（Av2）**：在控制 IP 先驗訊號（E1）後，加入 Text 與 Image（E3）能帶來具統計顯著性的效能提升（預期 MAE 降低至少 5% 以上）。

---

### 6. 評估指標

#### 為什麼 Popularity 和 Score 指標不對稱？

Popularity 為長尾連續量，真實數值可能跨越多個數量級（例如從數百到數十萬），且商業決策高度依賴「排序與相對量級」。因此需要 MAPE 來處理不同量級作品的相對誤差，並需要 Pearson/Spearman 評估排序關聯。反之，Score 分布集中（常態分佈）、絕對誤差具直接可解釋性（差 5 分即是 5 分差距），且目標為真實品質之評價解釋而非單純排序，因此採用 R² 來評估模型解釋變異量，避免引入不必要的指標（如 MAPE）。
...
|---|---|
| MAE | 主要指標 |
在此應用框架中，高／低的分界閾值（Threshold）將依據歷史訓練集資料的**分位數（如 Top 25% 定義為高）**，或採用產業基準（如 Score > 75 分即為高）來進行劃分。
