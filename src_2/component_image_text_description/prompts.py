"""
ToriiGate-0.5 prompt templates（來源：Minthy/ToriiGate-0.5/scripts/prompts.py）
"""
import random
...
def make_user_query(
    ...
    """
    item 可包含的欄位：
        tags             : list[str]  booru tags
        characters       : list[str]  角色名稱
        char_p_tags      : dict       {'chars': {name: [tags]}, 'skins': {name: [tags]}}
        char_descr       : dict       {'chars': {name: descr},  'skins': {name: descr}}
    """
    tags = item.get('tags', [])
    random.shuffle(tags)
            ...
            if len(chars_popular_tags['chars']) > 0 and (add_char_tags or add_description):
                ...
                if add_char_tags:
                    ...
                    for c_name, c_tags in chars_popular_tags['chars'].items():
                        ...
                        tags_s = (', '.join([a.replace('_', ' ') if len(a) > 3 else a for a in c_tags])
                                  if char_underscores else ' '.join(c_tags))
