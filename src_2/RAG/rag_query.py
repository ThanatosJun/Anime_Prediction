"""
查詢 RAG collection，為每個 split 的每部動畫找到 top-k 相似動畫

輸出（parquet）：
  id, rag_title_romaji, rag_popularity, rag_score, rag_release_year, rag_episodes,
  rag_genres, rag_format, rag_studios, rag_found, retrieved_ids
...
def query_split(
    client, encoder, df, split,
    text_emb_map, image_emb_map,
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"RAG query [{split}]"):
        ...
        if text_vec is not None:
            prefetch.append(models.Prefetch(
                query=text_vec.tolist(), using="text",
                limit=prefetch_k, filter=query_filter,
            ))
        if image_vec is not None:
            prefetch.append(models.Prefetch(
                query=image_vec.tolist(), using="image",
                limit=prefetch_k, filter=query_filter,
            ))
...
def query_all_splits(splits=("train", "val", "test")):
    ...
    for split in splits:
        ...
        text_emb_map  = _load_emb_map(str(Path(text_emb_dir)  / f"text_embeddings_{split}.parquet"),  "emb_")
        image_emb_map = _load_emb_map(str(Path(image_emb_dir) / f"image_embeddings_{split}.parquet"), "img_") if image_emb_dir else {}
