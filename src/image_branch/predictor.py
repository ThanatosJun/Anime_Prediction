...
def _infer_col(model, df: pd.DataFrame, image_dir: str,
    ...
    """Returns {anime_id: np.array(768)} for all rows in df."""
    ...
    with torch.no_grad():
        for orig, _, idxs in loader:
            orig = orig.to(device)
            embs = get_embedding(model, orig)          # (B, 768)
...
def predict(model, config: dict, device) -> None:
    ...
    mat = np.stack([embs[i] for i in ids], axis=0)   # (N, 768)
