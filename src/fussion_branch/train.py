...
def train_one_target(config: dict, target_col: str) -> dict:
    ...
    if Path(encoder_path).exists():
        ...
        meta_train = pd.read_csv(f"{cfg_data['fusion_meta_dir']}/fusion_meta_train.csv")
    ...
    input_dim = train_ds.feature_dim   # text(384) + image(768 or 0) + meta_rag(158)
    model = FusionMLP(
        input_dim=input_dim,
    ...
    print(f"  Input dim: {input_dim}  (text=384, image={'768' if use_img else '0 (zeros)'}, meta_rag={encoder.feature_dim})")
