...
Feature layout (in order):
  ...
  [one-hot format]     ? dims  fit from training vocab
  [one-hot source]     ? dims  fit from training vocab
  [one-hot country]    ? dims  fit from training vocab
  [binary]             3 dims  isAdult, is_sequel, has_sequel
  [genres multi-hot]  19 dims
  [studios multi-hot] 50 dims  top-K by frequency
  [rag numerical]      3 dims  rag_popularity, rag_score, rag_release_year (standardized)
  [rag_found]          1 dim
...
class MetaEncoder:
    ...
    def _update_dim(self):
        self.feature_dim = (
            ...
            + 1                               # rag_found
            + len(self.studio_vocab)          # rag studios
        )
