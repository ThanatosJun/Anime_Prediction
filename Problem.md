...
面對Concept Drift，我們使用最簡單的 linear 的預測減緩popularity 以及 meanScore 目標值在不同年度間的偏差，但從train, validation, and test set 的結果可以得知，這項情況仍然沒有解決。若要進一步減輕差異，除了使用最新的資料訓練外，我們應該嘗試非線性模型預測應對指數變動的數值。若要完全解決數據變動的問題，我們應該使用相對排名，不要預測目標的絕對數值，而是預測相對排序。此外因為我們使用的訓練資料集為 AniList 平台資料，使用者大多偏向歐美人士，因此無法代表台灣使用者對於動畫喜好的趨勢。